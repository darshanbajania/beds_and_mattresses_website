...
def Home_view(request):
    ...
    return render(request,'sleeping_beds/base.html',context)

#products home page
def Home_products_view(request):


    return render(request,'sleeping_beds/home_products.html')
...
def Product_view(request):
    ...
    if request.method == "POST":
        if request.session.has_key('is_logged_in'):
            ...
            return redirect('sleeping_beds:login_view')
    if request.session.has_key('is_logged_in'):
        current_customer =  Customers.objects.filter(user=  request.user).first()
        purchased_items = len(eval(current_customer.currently_purchased_products))
    context = {
...
def About_us_view(request):
    ...
    return render(request,'sleeping_beds/about.html',context)
...
def Cart_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        if customer_a.address == "address":
            return redirect('sleeping_beds:account_view')
        main_dictionary={}
        total_list=0
        no_products = True
        if request.method == "POST":
        ...
        return render(request,'sleeping_beds/cart.html',context)
    else:
        return redirect('sleeping_beds:home')     
...
def Checkout_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        return render(request,'sleeping_beds/checkout.html',context) 
    else:
        return redirect('sleeping_beds:login_view')
...
def Customer_profile_view(request):
    return render(request,'sleeping_beds/customer_profile.html')
...
def Customer_purchase_details_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        print(now)
        return render(request,'sleeping_beds/customer_purchase_details.html',context)
    else:
        return redirect('sleeping_beds:home')
...
def Accounts_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        return render(request, 'sleeping_beds/customer_account.html',context)
    else:
        return redirect('sleeping_beds:home')
...
def Admin_dashboard_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        if request.method == 'POST':
            ...
            base_url = reverse('sleeping_beds:admin_customer_details')
            # creating a string of dictionary
        ...
        return render(request,'sleeping_beds/admin_dashboard.html',context)
    else:
        return redirect('sleeping_beds:home')
...
def Admin_customer_details_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        return render(request, 'sleeping_beds/admin_customer_details.html',context)
    else:
        return redirect('sleeping_beds:home')
...
def Admin_product_update_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        return render(request,'sleeping_beds/admin_product_update.html',context)
    else:
        return redirect('sleeping_beds:home')
...
def Admin_products_detail_update_view(request):
    if request.session.has_key('is_logged_in'):
        ...
        return render(request, 'sleeping_beds/admin_products_detail_update.html',context)
    else:
        return redirect('sleeping_beds:home')
